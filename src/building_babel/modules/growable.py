...
class GrowableRMSNorm(nn.Module):
    ...
    def mask_update(self):
        ...
        self.iter += 1
        self.mask_frac = (
            self.dims[-2] + (self.dims[-1] - self.dims[-2]) * self.iter / self.num_iters
        )
    ...
    def _norm(self, x):
        ...
        if self.iter >= self.num_iters:
            ...
            for i, (prev_dim, dim) in enumerate(zip([0] + self.dims, self.dims, strict=False)):
...
class GrowableLinear(nn.Module):
    ...
    def grow(
        self, final_in_dim: int, final_out_dim: int, init: InitFunc_ | None = None
    ):
        """
        ...
        """
        assert (
            final_in_dim > self.in_dims[-1] + 1
        ), "Must increase in size by more than 1"
        assert (
            final_out_dim > self.out_dims[-1] + 1 or final_out_dim == self.out_dims[-1]
        ), "Must increase out size by more than 1, or zero"
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            raise KeyError("key must be a n,m tuple") from None
        for i, (prev_in_dim, in_dim, prev_out_dim, out_dim) in enumerate(
            zip([0] + self.in_dims, self.in_dims, [0] + self.out_dims, self.out_dims, strict=False)
        ):
    ...
    def forward(self, x):
        ...
        for i, (prev_in_dim, in_dim, prev_out_dim, out_dim) in enumerate(
            ...
            if i == 0:
                ...
                y[..., prev_out_dim:out_dim] += F.linear(
                    x[..., :prev_in_dim], self.weight_splits[str(i)]["ll"]
                )
                y[..., :prev_out_dim] += F.linear(
                    x[..., prev_in_dim:in_dim], self.weight_splits[str(i)]["ur"]
                )
                y[..., prev_out_dim:out_dim] += F.linear(
                    x[..., prev_in_dim:in_dim], self.weight_splits[str(i)]["lr"]
                )
        return y
